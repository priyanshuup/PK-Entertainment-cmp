...
.then((data)=>{
  ...
  videoIframeevents.src = `https://www.youtube.com/embed/${eventsdataApi[0].url}?autoplay=1&mute=1&loop=1&playlist=${eventsdataApi[0].url}`; // Assuming the 'title' property contains the YouTube video URL
...
.then((data) => {
  ...
  rentsapidata.forEach((item) => {
    ...
    const headingRent = document.createElement("h1");
